...
kind = "생리장해"
move_path = r"D:\CropFairy\AI\tomato\images\train"
folder_path = f"{search_path}\\{kind}"
targets = os.listdir(folder_path)

# move_image = {"b1": {11: {3: [1, 2, 3]}}
move_image = {"b7":{13:{3:[1,2,3]}}, "b8":{11:{3:[1,2,3]}, 12:{3:[1,2,3]}}}

move_count = [160, 160, 80, 150, 150, 150, 250, 150, 150]
move_num = 0
    ...
    for grow in move_image[disease].keys():
        for area in move_image[disease][grow].keys():
            for risk in move_image[disease][grow][area]:
                print(f"------------------ 2_2_{disease}_{area}_2_{grow}_{risk}")
                image_list = list()
